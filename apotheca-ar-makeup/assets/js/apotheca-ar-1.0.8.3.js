...
(function ($) {
  ...
  // Some browsers (notably Safari) cannot decode SVG blobs via createImageBitmap.
  // Provide a robust fallback that rasterises via an <img> and a temporary canvas.
  function _rasteriseSvgToDrawable(svgText) {
      ...
      try {
        ...
        if (typeof createImageBitmap === 'function') {
          createImageBitmap(blob).then((bitmap) => {
            resolve({ bitmap, w: bitmap.width, h: bitmap.height });
          }).catch(() => {
            ...
            img.onload = function () {
              try {
                ...
                // Canvas itself is drawable by drawImage.
                resolve({ bitmap: c, w, h });
              } catch (e) {
                URL.revokeObjectURL(url);
                resolve(null);
              }
        ...
        // No createImageBitmap available: use <img> decode.
        const url = URL.createObjectURL(blob);
        const img = new Image();
        img.onload = function () {
          const w = img.naturalWidth || img.width || 512;
  ...
  function _svgToEyelashBitmap(color, flipX) {
    ...
    return _rasteriseSvgToDrawable(svg).then((obj) => {
      if (obj && obj.bitmap) {
        _eyelashCache.set(key, obj);
        return obj;
      }
      console.warn('[Apotheca AR] Failed to rasterise eyelash SVG');
      return null;
    });
  }
